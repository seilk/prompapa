...
def _anthropic_resp(content: str) -> MagicMock:
    m = MagicMock(spec=httpx.Response)
    m.raise_for_status = MagicMock()
    m.json.return_value = {"content": [{"text": content}]}
    return m


def _google_resp(content: str) -> MagicMock:
    m = MagicMock(spec=httpx.Response)
    m.raise_for_status = MagicMock()
    m.json.return_value = {"data": {"translations": [{"translatedText": content}]}}
    return m


# ── Google provider ────────────────────────────────────────────────────────────

async def test_google_translates_korean(monkeypatch):
    monkeypatch.setenv("GOOGLE_API_KEY", "fake-google-key")
    cfg = _cfg(provider="google", model="", api_key_env="GOOGLE_API_KEY")
    with patch("httpx.AsyncClient", return_value=_mock_client(
        AsyncMock(return_value=_google_resp("Fix this bug.")))):
        result = await rewrite_to_english("이 버그 고쳐줘", cfg)
    assert result == "Fix this bug."


async def test_google_api_error_raises(monkeypatch):
    monkeypatch.setenv("GOOGLE_API_KEY", "fake-google-key")
    cfg = _cfg(provider="google", model="", api_key_env="GOOGLE_API_KEY")
    with patch("httpx.AsyncClient", return_value=_mock_client(
        ...
        with pytest.raises(TranslationError, match="403"):
            await rewrite_to_english("테스트", cfg)


async def test_google_timeout_raises(monkeypatch):
    monkeypatch.setenv("GOOGLE_API_KEY", "fake-google-key")
    cfg = _cfg(provider="google", model="", api_key_env="GOOGLE_API_KEY")
    with patch("httpx.AsyncClient", return_value=_mock_client(
...
async def test_google_uses_correct_endpoint(monkeypatch):
    monkeypatch.setenv("GOOGLE_API_KEY", "fake-google-key")
    cfg = _cfg(provider="google", model="", api_key_env="GOOGLE_API_KEY")
    mock_post = AsyncMock(return_value=_google_resp("Hello."))
    with patch("httpx.AsyncClient", return_value=_mock_client(mock_post)):
        await rewrite_to_english("안녕", cfg)
    url_called = mock_post.call_args[0][0]
    assert "translation.googleapis.com" in url_called
...
async def test_korean_text_returns_english(monkeypatch):
    ...
    with patch("httpx.AsyncClient", return_value=_mock_client(
        AsyncMock(return_value=_openai_resp(expected)))):
...
async def test_strips_whitespace_from_response(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-fake")
    with patch("httpx.AsyncClient", return_value=_mock_client(
        AsyncMock(return_value=_openai_resp("  Fix this bug.  \n")))):
...
async def test_anthropic_provider_uses_correct_endpoint(monkeypatch):
    ...
    cfg = _cfg(provider="anthropic", model="claude-3-haiku-20240307",
               api_key_env="ANTHROPIC_API_KEY")
    mock_post = AsyncMock(return_value=_anthropic_resp("Fix the bug."))
    with patch("httpx.AsyncClient", return_value=_mock_client(mock_post)):
