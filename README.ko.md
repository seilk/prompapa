...
> **효율성에 대한 참고:** 비영어 언어는 심각한 토큰 페널티를 받습니다. 예를 들어 한국어는 영문으로 똑같은 의미를 표현할 때보다 종종 2~3배 더 많은 토큰을 소모합니다. 제출 전에 번역을 거치면 컨텍스트 윈도우 팽창(context window bloat)과 API 비용을 크게 줄일 수 있습니다.
...
TUI를 실행하지 않고 커맨드 라인에서 텍스트를 직접 번역할 수 있습니다:

```bash
papa -t "버그를 고쳐줘"          # 한국어  → Fix the bug.
papa -t "バグを修正して"          # 일본어 → Fix the bug.
papa -t "修复这个错误"            # 중국어  → Fix this error.
papa -t "Corrige el error"       # 스페인어  → Fix the error.
papa -t "Corrige le bug"         # 프랑스어   → Fix the bug.
papa -t "Behebe den Fehler"      # 독일어   → Fix the bug.
```

번역 결과는 표준 출력(stdout)으로 인쇄되므로 다른 도구로 쉽게 파이핑할 수 있습니다:
...
- [ ] OpenCode 사이드바 뷰 지원 — 현재 사이드바가 활성화되어 있으면 오류가 발생함
